...
export async function updatePlaylist(user: DiscordUsers, message: Message) {
  //   let limit = 100;
  //   let result: string[][] = [];
  //   let uris: string[] = [];
  //   let z = 0;
  ...
  //   for (let j in result) {
  //     console.log(j);
  //     addSongs(message as SpotifyCredentials, result[j] as Message<boolean>); // Cast 'result[j]' to 'Message<boolean>'
  //     addSongs(message as SpotifyCredentials, result[j] as Message<boolean>); // Cast 'result[j]' to 'Message<boolean>'
  //   }
  //   console.log(result);
  // ``
